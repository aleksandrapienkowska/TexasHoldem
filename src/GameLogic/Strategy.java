...
public class Strategy {
	...
	public int[] Winner(int besthands[][], Cards[] common, Cards pCards[][]) {
		...
		if (remis) {
			...
			if (besthands[remistab[0]][6] == 4
					|| besthands[remistab[0]][6] == 5) {
				// KOLOR + street
				int a = 1;
				int win = 0;
				List<Integer> winlist = new ArrayList<Integer>();
				...
				for (a = 1; a < best.length; a++) {
					for (b = best[0].length - 1; b >= 0 && a < best.length; b--) {
						// System.out.println(a+" "+b+" "+win+" "+best[win][b].wartosc+
						// " "+best[a][b].wartosc);
						...
						} else if (best[win][b].wartosc > best[a][b].wartosc) {
			...
			if (besthands[remistab[0]][6] == 6) {
				...
				if (winid.size() == 1) {
					...
					for (int j = 1; j < winid.size(); j++) {
						// System.out.println(winlist+" PRzed win"+winid.get(win)+
						// " j "+winid.get(j));
						if (idthree.get(winid.get(win)) > idthree.get(winid
								.get(j))) {
							...
							if (idthree.get(winid.get(win)) == idthree
									.get(winid.get(j))) {
								if (idthree.get(winid.get(win)) == 2) {
									if (best[winid.get(win)][1].wartosc > best[winid
											.get(j)][1].wartosc) {
										// win
										...
										winner[winner.length - 1] = 1;
									} else if (best[winid.get(win)][1].wartosc < best[winid
											.get(j)][1].wartosc) {
										// j
										// System.out.println("win=2 win1<j1");
										winlist.remove(winid.get(win));
										...
										winlist.add(winid.get(win));
											...
											.get(j)][1].wartosc) {
										// System.out.println("win=2 win1=j1");
										if (best[winid.get(win)][0].wartosc > best[winid
												.get(j)][0].wartosc) {
											// win
											...
											winner[winner.length - 1] = 1;
											winlist.add(winid.get(win));
										} else if (best[winid.get(win)][0].wartosc < best[winid
												.get(j)][0].wartosc) {
											// j
											// System.out.println("win=2 win0<j0");
											winlist.remove(winid.get(win));
								...
								} else if (idthree.get(winid.get(win)) == 1) {
									// System.out.println("win=1 ");
									if (best[winid.get(win)][4].wartosc > best[winid
											.get(j)][4].wartosc) {
										// win
										...
										winner[winner.length - 1] = 1;
										winlist.add(winid.get(win));
									} else if (best[winid.get(win)][4].wartosc < best[winid
											.get(j)][4].wartosc) {
										// j
										// System.out.println("win4<j4");
										winlist.remove(winid.get(win));
								...
								} else if (idthree.get(winid.get(win)) == 0) {
									...
									if (best[winid.get(win)][4].wartosc > best[winid
										...
										winlist.add(winid.get(win));
											...
											.get(j)][4].wartosc) {
										// System.out.println("win4=j4");
										if (best[winid.get(win)][3].wartosc > best[winid
												.get(j)][3].wartosc) {
											// win wygral
											...
											winner[winner.length - 1] = 1;
											winlist.add(winid.get(win));
										} else if (best[winid.get(win)][3].wartosc < best[winid
												.get(j)][3].wartosc) {
											// j wygral
											// System.out.println("win3<j3");
											winlist.remove(winid.get(win));
							...
							} else if (idthree.get(winid.get(win)) > idthree
									.get(winid.get(j))) {
						...
						if (idtab[i][0] > idtab[win][0]) {
							...
							if (idtab[i][0] == 3) {
								...
								for (int a = 2; a >= 0; a--) {
									...
									System.out.println(best[idtab[i][1]][a]
											+ "  " + best[idtab[win][1]][a]);
	...
	public static void main(String args[]) {
		...
		Cards comm[] = { Cards.DZIESIEC_a, Cards.DZIEWIEC_a, Cards.KING_a,
				Cards.SZESC_d, Cards.TRZY_b };
		pCards[0][0] = Cards.AS_b;
		pCards[0][1] = Cards.CZTERY_b;
		pCards[1][0] = Cards.DZIESIEC_b;
		pCards[1][1] = Cards.PIEC_c;
		pCards[2][0] = Cards.CZTERY_a;
		pCards[2][1] = Cards.TRZY_a;
		pCards[3][0] = Cards.QUEEN_c;
		pCards[3][1] = Cards.JOPEK_d;
		pCards[4][0] = Cards.SZESC_c;
		pCards[4][1] = Cards.SZESC_b;
		pCards[5][0] = Cards.DZIEWIEC_b;
		pCards[5][1] = Cards.KING_c;
		pCards[6][0] = Cards.QUEEN_a;
		pCards[6][1] = Cards.JOPEK_a;
		System.out.println("graczy    " + players);
		Strategy St = new Strategy(players, pCards, comm);

		int[][] best = St.BestHands();
		for (int i = 0; i < best.length; i++) {
		...
		Cards comm2[] = { Cards.SIEDEM_a, Cards.SIEDEM_b, Cards.DWA_c,
				Cards.SZESC_d, Cards.TRZY_b };
		pCards2[1][0] = Cards.SIEDEM_c;
		pCards2[1][1] = Cards.SIEDEM_d;
		pCards2[0][0] = Cards.DWA_b;
		pCards2[0][1] = Cards.DWA_d;
		Strategy St2 = new Strategy(players2, pCards2, comm2);
		System.out.println("graczy    " + players2);
		int[][] best2 = St2.BestHands();
		for (int i = 0; i < best2.length; i++) {
			for (int k = 0; k < best2[0].length; k++) {
				System.out.print(best2[i][k] + "  ");
			}
			System.out.println("");
		}
		System.out.println("");
		St.Winner(best, comm, pCards);
		System.out.println("");
		System.out.println("");
		St2.Winner(best2, comm2, pCards2);
		System.out.println("");
