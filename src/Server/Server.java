				...
				try {
					...
					for (i = 0; i < maxClientsCount; i++) {
						if (threads[i] == null) {
							...
							os.println(i);
							os.println("|Zostałeś podłączony do gry poczekaj na resztę graczy.|Twoje id to "
									+ i);
					...
					if (i == maxClientsCount - 1) {
						...
						for (int k = 0; k < threads.length; k++) {
							threads[k].out
									.println("|Wszyscy gracze dołączyli do gry. Rozpoczynamy rozgrywkę!!!|");
							response = table.getResponse();
							int[] hand = table.getHand(k);
							threads[k].out
									.println("setBill" + table.getCash(k));
							threads[k].out.println("setPot" + table.getPot());
							threads[k].out.println("setMaxBet"
									+ table.getMaxBet());
		...
		try {
			...
			while (true) {
				...
				if (data != null) {
					...
					String message = "";
					if (table.getCurrent() == Id && table.checkActive(Id)) {
						OutputData[0] = Id;
						...
						case "be": {
							...
							if (table.active_players != 1)
								if ((OutputData[3] > table.getCash(Id) || table
										.getMaxBet() != 0)) {
									OutputData[3] = -2;
								}
							break;
						...
						if (OutputData[3] >= -1) {
							...
							table.listen(Output);
							threads[Id].out.println("setBill"
									+ table.getCash(Id) + "");
					...
					synchronized (this) {
						...
						for (int z = 0; z < threads.length; z++) {
							...
							threads[z].out.println("setPot" + table.getPot()
									+ "");
							threads[z].out.println("setMaxBet"
									+ table.getMaxBet() + "");
								...
								while ((message.substring(0, 4))
									...
									for (int b = 0; b < threads.length; b++) {
										...
										for (int a = 0; a < temp.length - 1; a++) {
											threads[b].out.println("setCommon"
													+ temp[a]);
										}
